...
namespace
{
    bool isLocalSplashLoginPage(const std::string& login_page)
    {
        return login_page.find("://") == std::string::npos ||
            login_page.compare(0, 7, "file://") == 0;
    }

    std::string makeFileURL(std::string local_path)
    {
        std::replace(local_path.begin(), local_path.end(), '\\', '/');
        return "file:///" + LLURI::escape(local_path, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~:/");
    }
    ...
    bool writeTextFile(const std::string& filename, const std::string& contents)
    {
        std::ofstream output(filename.c_str(), std::ios::out | std::ios::binary | std::ios::trunc);
        if (!output.is_open())
        {
            return false;
        }
    ...
    std::string getLocalSplashPath(const std::string& login_page)
    {
        if (login_page.compare(0, 7, "file://") == 0)
        {
            std::string local_path = login_page;
            if (local_path.compare(0, 8, "file:///") == 0)
            {
                local_path = local_path.substr(8);
            }
            else
            {
        ...
        if (local_path.empty())
        {
            local_path = "splash/index.html";
        }

        if (local_path.size() < 2 || local_path[1] != ':')
        {
            local_path = gDirUtilp->getExpandedFilename(LL_PATH_EXECUTABLE, local_path);
        }

        return local_path;
    }

    std::string makeLocalSplashLoginURL(const std::string& login_page)
    {
        std::string index_path = getLocalSplashPath(login_page);
        std::string splash_dir = index_path;
        std::string::size_type slash_pos = splash_dir.find_last_of("\\/");
        if (slash_pos != std::string::npos)
        {
            splash_dir = splash_dir.substr(0, slash_pos);
        }

        std::string index_html = readTextFile(index_path);
        std::string splash_xml = readTextFile(gDirUtilp->add(splash_dir, "splash.xml"));
        if (index_html.empty() || splash_xml.empty())
        {
            return makeFileURL(index_path);
        }

        const std::string base_marker = "<head>";
        const std::string script_marker = "<script src=\"splash.js\"></script>";
        const std::string base_tag = "<head>\n  <base href=\"" + makeFileURL(splash_dir + gDirUtilp->getDirDelimiter()) + "\">";
        const std::string config_script = "<script>window.WAVEBREAKER_SPLASH_XML=\"" + escapeJavaScriptString(splash_xml) + "\";</script>\n  " + script_marker;

        std::string::size_type base_pos = index_html.find(base_marker);
        if (base_pos != std::string::npos)
        {
            index_html.replace(base_pos, base_marker.size(), base_tag);
        }

        std::string::size_type script_pos = index_html.find(script_marker);
        if (script_pos != std::string::npos)
        {
            index_html.replace(script_pos, script_marker.size(), config_script);
        }

        std::string generated_path = gDirUtilp->getExpandedFilename(LL_PATH_CACHE, "wavebreaker_login_splash.html");
        if (!writeTextFile(generated_path, index_html))
        {
            LL_WARNS("AppInit") << "Unable to write generated local splash page: " << generated_path << LL_ENDL;
            return makeFileURL(index_path);
        }

        return makeFileURL(generated_path);
    }
}
...
void FSPanelLogin::loadLoginPage()
{
    ...
    std::string login_page_setting = LLGridManager::getInstance()->getLoginPage();
    LLURI login_page = LLURI(login_page_setting);
    ...
    if ( force_login_url.length() > 0 )
    {
        ...
        login_page_setting = force_login_url;
    ...
    std::string login_uri_string;
    if (isLocalSplashLoginPage(login_page_setting))
    {
        login_uri_string = makeLocalSplashLoginURL(login_page_setting) + LLURI::mapToQueryString(params);
    }
    else
    {
        ...
        LLURI login_uri(LLURI::buildHTTP(url,
                                         login_page.path(),
                                         params));
        login_uri_string = login_uri.asString();
    }

    LLMediaCtrl* web_browser = sInstance->getChild<LLMediaCtrl>("login_html");
    if (web_browser->getCurrentNavUrl() != login_uri_string)
    {
        LL_DEBUGS("AppInit") << "loading:    " << login_uri_string << LL_ENDL;
        web_browser->navigateTo(login_uri_string, HTTP_CONTENT_TEXT_HTML, isLocalSplashLoginPage(login_page_setting));
    }
}
...
#endif // OPENSIM
            // <FS:PP> Load Wavebreaker's registration page from within the viewer itself
