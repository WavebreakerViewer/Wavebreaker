...
class FSViewerManifest:
    def fs_installer_basename(self):
        ...
        return "Wavebreaker-%(app_name)s_%(optimized)s-%(version_dashes)s" % substitution_strings
    ...
    def fs_channel_unique(self):
        return self.channel().replace("Wavebreaker", "").strip()
    ...
    def fs_sign_win_binaries(self):
        ...
        executable_paths = [
            # self.args['configuration'] + "\\wavebreaker-bin.exe", # no need to sign this we are not packaging it.
    ...
    def fs_linux_tar_excludes(self):
        installer_name_components = ['Wavebreaker',self.app_name(),self.args.get('arch'),'.'.join(self.args['version'])]
    ...
    def fs_save_windows_symbols(self):
        ...
        pdbName = "wavebreaker-bin.pdb"
        try:
            subprocess.check_call( [ "pdbcopy.exe" ,
                                     self.args['configuration'] + "\\wavebreaker-bin.pdb",
                                     self.args['configuration'] + "\\wavebreaker-bin-public.pdb",
                                     "-p"
                                 ], stderr=subprocess.PIPE,stdout=subprocess.PIPE )
            pdbName = "wavebreaker-bin-public.pdb"
        except:
            print("Cannot run pdbcopy, packaging private symbols")

        tarName = "%s/Wavebreaker_%s_%s_%s_pdbsymbols-windows-%d.tar.xz" % (self.args['configuration'].lower(),
        ...
        symbolTar.add( "%s/Wavebreaker-bin.exe" % self.args['configuration'].lower(), "wavebreaker-bin.exe" )
    ...
    def fs_save_symbols(self, osname):
        if (os.path.exists("%s/wavebreaker-symbols-%s-%d.tar.bz2" % (self.args['configuration'].lower(),
                                                                       osname,
                                                                       self.address_size))):
            # Rename to add version numbers
            sName = "%s/Wavebreaker_%s_%s_%s_symbols-%s-%d.tar.bz2" % (self.args['configuration'].lower(),
                                                                       self.fs_channel_legacy_oneword(),
                                                                       '-'.join( self.args['version'] ),
                                                                       self.args['viewer_flavor'],
                                                                       osname,
            ...
            os.rename("%s/wavebreaker-symbols-%s-%d.tar.bz2" % (self.args['configuration'].lower(), osname, self.address_size), sName)
    ...
    def fs_save_breakpad_symbols(self, osname):
        ...
        components = ['Wavebreaker',self.app_name(),self.args.get('arch'),'.'.join(self.args['version'])]
