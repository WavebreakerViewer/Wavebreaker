 ...
 * @brief SLurl command handler for Wavebreaker commands
 *
...
public:
    ...
    FSSlurlCommandHandler() : LLCommandHandler("wavebreaker", UNTRUSTED_BLOCK) { }
...
class FSWavebreakerExternalLinkHandler : public LLCommandHandler
{
public:
    FSWavebreakerExternalLinkHandler() : LLCommandHandler("wavebreakerlink", UNTRUSTED_CLICK_ONLY) { }

    bool handle(const LLSD& params, const LLSD& query_map, const std::string& grid, LLMediaCtrl* web)
    {
        if (params.size() < 1 || params[0].asString() != "open")
        {
            return false;
        }

        const std::string url = query_map["url"].asString();
        if (url.empty()
            || (!LLStringUtil::startsWith(url, "https://")
                && !LLStringUtil::startsWith(url, "http://")))
        {
            return false;
        }
...
// Creating the object registers with the dispatcher.
FSWavebreakerExternalLinkHandler gFSWavebreakerExternalLinkHandler;
