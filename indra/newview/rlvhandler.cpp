...
bool RlvHandler::handleEvent(LLPointer<LLOldEvents::LLEvent> event, const LLSD& sdUserdata)
{
    ...
    //           will still get us the custom "update grouptitle list" Wavebreaker events, sending us
    //           into a toggle-loop!
...
// Handles: @setcam_avdistmax:<distance>=n|y and @camdistmax:<distance>=n|y changes
template<>
void RlvBehaviourModifierHandler<RLV_MODIFIER_SETCAM_AVDISTMAX>::onValueChange() const
{
    if ( (RlvActions::isCameraForcedMouselook()) && (!gAgentCamera.cameraMouselook()) )
