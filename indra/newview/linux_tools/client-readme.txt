Wavebreaker - Linux Beta README
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

This document contains information about the Wavebreaker Linux
client
...
Hi!  This is a BETA release of the Wavebreaker client for Linux.
...
Minimum requirements:
    ...
    * Video/Graphics Card:
      ...
      **NOTE**: Wavebreaker absolutely requires you to have recent, correctly-
      ...
      TROUBLESHOOTING section if you encounter problems starting Wavebreaker.
...
The Wavebreaker Linux client can entirely run from the directory you have
...
Run './wavebreaker' from the installation directory to start Wavebreaker.
...
User data is stored in the hidden directory ~/.wavebreaker by default; you may
override this location with the WAVEBREAKER_USER_DIR, or WAVEBREAKER_X64_USER_DIR
...
run from.  Most of this is also replicated in ~/.wavebreaker/logs/Wavebreaker.log
...
PROBLEM 1:- Wavebreaker fails to start up, with a warning on the console like:
 ...
 * If your distribution does not make it easy, then you can download the
   ...
   - <https://wiki.wavebreakerviewer.org/downloads#video_drivers>

PROBLEM 2:- My whole system seems to hang when running Wavebreaker.
...
SOLUTION:- Some residents with ATI cards have reported that running
   'sudo aticonfig --locked-userpages=off' before running Wavebreaker solves
   their stability issues.
SOLUTION:- As a last resort, you can disable most of Wavebreaker's advanced
   graphics features by editing the 'wavebreaker' script and removing the '#'
   from the line which reads '#export LL_GL_NOEXT=x'

PROBLEM 3:- After I minimize the Wavebreaker window, it's just blank when
   it comes back.
SOLUTION:- Some Linux desktop 'Visual Effects' features are incompatible
   with Wavebreaker.  One reported solution is to use your desktop
...
SOLUTION:- The most common solution is to ensure that you have the 'esd'
   ...
   start Wavebreaker.  Users of Ubuntu (and some other) Linux systems can
...
SOLUTION:- Some window managers eat the Alt key for their own purposes; you
   ...
   properly with mouse actions in Wavebreaker and other applications.
...
SOLUTION:- You need to have a working installation of GStreamer 0.10; this
   ...
   volume control / muting to fully function inside Wavebreaker.


6. ADVANCED TROUBLESHOOTING
-=-=-=-=-=-=-=-=-=-=-=-=-=-

The 'wavebreaker' script which launches Wavebreaker contains some
...
* AUDIO - Edit the 'secondlife' script and you will see these audio
  ...
  Wavebreaker tries to use OpenAL, FMODSTUDIO (PULSEAUDIO, ALSA)
...
We're pleased to have released Wavebreaker client's source code under
an Open Source license compatible with the 'LGPL'.
<https://wiki.wavebreakerviewer.org/downloads#source_code>


8. GETTING MORE HELP AND REPORTING PROBLEMS
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

For general help and support with Wavebreaker:
<http://wiki.wavebreakerviewer.org>

In-world discussion: 

Main support: Join one of our in-world support groups.
<http://wiki.wavebreakerviewer.org/wavebreaker_in-world_groups>
...
The Wavebreaker Issue Tracker:
<https://jira.wavebreakerviewer.org/>
This is the right place for finding known issues and reporting new
bugs in all Wavebreaker releases if you find that the Troubleshooting
