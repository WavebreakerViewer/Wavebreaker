...
bool LLFolderBridge::isItemMovable() const
{
    ...
    if(obj)
    {
        ...
        // <FS:Ansariel> FIRE-28977: Lock special and locked folders from being DaD'ed
        if (obj->getName() == ROOT_WAVEBREAKER_FOLDER || obj->getName() == RLV_ROOT_FOLDER || isLockedFolder())
        {
            return false;
        }
        // </FS:Ansariel>
...
LLFolderType::EType LLFolderBridge::getPreferredType() const
{
    ...
    if (LLViewerInventoryCategory* cat = getCategory())
    {
        ...
        if (catName == ROOT_WAVEBREAKER_FOLDER) preferred_type = LLFolderType::FT_WAVEBREAKER;
        else if (catName == RLV_ROOT_FOLDER) preferred_type = LLFolderType::FT_RLV;
        else if (catName == "#Wavebreaker Legacy") preferred_type = LLFolderType::FT_WAVEBREAKER_LEGACY;
...
EInventorySortGroup LLFolderBridge::getSortGroup() const
{
    ...
    if (!sFSSortFSFoldersToTopPt)
    {
        ...
        {
            std::string catName(cat->getName());
            if ((catName == ROOT_WAVEBREAKER_FOLDER) || (catName == RLV_ROOT_FOLDER) || (catName == "#Wavebreaker Legacy"))
            {
...
void LLObjectBridge::performAction(LLInventoryModel* model, std::string action)
{
    if (isAddAction(action))
    {
        ...
        {
            // <FS> Wavebreaker's replace on double click
