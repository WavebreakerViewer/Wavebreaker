...
void AOEngine::tick()
{
    ...
    const LLUUID categoryID = gInventory.findCategoryByName(ROOT_WAVEBREAKER_FOLDER);

    if (categoryID.isNull())
    {
        LL_WARNS("AOEngine") << "no " << ROOT_WAVEBREAKER_FOLDER << " folder yet. Creating ..." << LL_ENDL;
        gInventory.createNewCategory(gInventory.getRootFolderID(), LLFolderType::FT_NONE, ROOT_WAVEBREAKER_FOLDER);
        mAOFolder.setNull();
    }
    else
    {
