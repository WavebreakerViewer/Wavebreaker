...
LLColor4 LGGContactSets::colorize(const LLUUID& uuid, LLColor4 color, ContactSetType type) const
{
    ...
    else if (FSData::instance().isAgentFlag(uuid, FSData::CHAT_COLOR))
    {
        switch (type)
        {
            case ContactSetType::CHAT:
            case ContactSetType::IM:
                color = LLUIColorTable::instance().getColor("WavebreakerChatColor", LLColor4::red).get();
                break;
            case ContactSetType::TAG:
                color = LLUIColorTable::instance().getColor("NameTagWavebreaker", LLColor4::red).get();
                break;
            case ContactSetType::MINIMAP:
                color = LLUIColorTable::instance().getColor("MapAvatarWavebreakerColor", LLColor4::red).get();
                break;
            case ContactSetType::RADAR:
                // Do nothing
                break;
