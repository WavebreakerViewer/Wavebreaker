...
#!define MUI_ICON   "%%SOURCE%%\installers\windows\wavebreaker_icon_os.ico"
#!define MUI_UNICON "%%SOURCE%%\installers\windows\wavebreaker_icon_os.ico"
!define MUI_ICON   "%%SOURCE%%\installers\windows\wavebreaker_icon${ICON_SUFFIX}.ico"
!define MUI_UNICON "%%SOURCE%%\installers\windows\wavebreaker_icon${ICON_SUFFIX}.ico"
# </FS:Ansariel>
...
!define LINDEN_KEY      "SOFTWARE\Wavebreaker"
...
WriteINIStr		"$SMPROGRAMS\$INSTSHORTCUT\SL Create Account.url" \
				...
				"https://www.wavebreakerviewer.org/join-secondlife/"
...
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "Publisher" "Wavebreaker"
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "URLInfoAbout" "https://www.wavebreakerviewer.org"
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "URLUpdateInfo" "https://www.wavebreakerviewer.org/downloads"
WriteRegStr SHELL_CONTEXT "${MSUNINSTALL_KEY}" "HelpLink" "https://www.wavebreakerviewer.org/support"
...
# Delete files in \Users\<User>\AppData\Roaming\Wavebreaker
# Remove all settings files but leave any other .txt files to preserve the chat logs
    RMDir /r "$2\AppData\Roaming\Wavebreaker\logs"
    RMDir /r "$2\AppData\Roaming\Wavebreaker\browser_profile"
    RMDir /r "$2\AppData\Roaming\Wavebreaker\user_settings"
    Delete  "$2\AppData\Roaming\Wavebreaker\*.xml"
    Delete  "$2\AppData\Roaming\Wavebreaker\*.bmp"
    Delete  "$2\AppData\Roaming\Wavebreaker\search_history.txt"
    Delete  "$2\AppData\Roaming\Wavebreaker\plugin_cookies.txt"
    Delete  "$2\AppData\Roaming\Wavebreaker\typed_locations.txt"
# Delete files in \Users\<User>\AppData\Local\Wavebreaker
    ${If} ${ISOPENSIM} == "0"
        ${If} ${IS64BIT} == "0"
            RMDir /r "$2\AppData\Local\Wavebreaker"				#Delete the Havok cache folder
        ${Else}
            RMDir /r "$2\AppData\Local\Wavebreaker_x64"			#Delete the OpenSim cache folder
        ${EndIf}
    ${Else}
        ${If} ${IS64BIT} == "0"
            RMDir /r "$2\AppData\Local\WavebreakerOS"			#Delete the Havok cache folder
        ${Else}
            RMDir /r "$2\AppData\Local\WavebreakerOS_x64"		#Delete the OpenSim cache folder
        ${EndIf}
    ${EndIf}
