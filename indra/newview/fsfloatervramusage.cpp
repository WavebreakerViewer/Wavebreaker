 ...
 * Wavebreaker Viewer Source Code
 ...
 *
 * The Wavebreaker Project, Inc., 1831 Oakwood Drive, Fairmont, Minnesota 56031-3225 USA
 * http://www.wavebreakerviewer.org
 * $/LicenseInfo$
 */
...
constexpr F32 PROPERTIES_REQUEST_TIMEOUT   = 10.0f;
constexpr F32 PROPERY_REQUEST_INTERVAL     = 2.0f;
constexpr U32 PROPERTIES_MAX_REQUEST_COUNT = 250;
...
FSFloaterVRAMUsage::FSFloaterVRAMUsage(const LLSD& seed) : LLFloater(seed)
{
    mData           = new ImplData();
    mData->mList    = 0;
    mData->mPending = 0;
...
U32 FSFloaterVRAMUsage::calcTexturSize(LLViewerObject* aObject, std::ostream* aTooltip)
{
    ...
    for (U8 j = 0; j < numTEs; ++j)
    {
        ...
        if (aTooltip)
            *aTooltip << "TE: " << (U32)j << " tx: " << textureId << " w/h/c " << pTex->getFullWidth() << "/" << pTex->getFullHeight()
                      << "/" << (U32)pTex->getComponents() << std::endl;
...
void FSFloaterVRAMUsage::doRefresh()
{
    ...
    for (S32 i = 0; i < numObjects; ++i)
    {
        ...
        if (!pObj->mbCanSelect || pObj->getPCode() == LLViewerObject::LL_VO_SURFACE_PATCH ||
            pObj->getPCode() == LLViewerObject::LL_VO_SKY || pObj->getPCode() == LLViewerObject::LL_VO_WL_SKY ||
            pObj->getPCode() == LLViewerObject::LL_VO_VOID_WATER || pObj->getPCode() == LLViewerObject::LL_VO_WATER || pObj->isAvatar())
            continue;
