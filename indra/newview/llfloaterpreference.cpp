...
{
    LLNotificationsUtil::add("WavebreakerClearSettingsPrompt",LLSD(), LLSD(), callback_clear_settings);
}
...
enum WavebreakerLindenGridType
{
    WAVEBREAKER_LINDEN_GRID_NONE,
    WAVEBREAKER_LINDEN_GRID_AGNI,
    WAVEBREAKER_LINDEN_GRID_ADITI
};

static const char* const WAVEBREAKER_AGNI_SPLASH_PAGE = "splash/index.html";
static const char* const WAVEBREAKER_ADITI_SPLASH_PAGE = "splash/index.html";

static WavebreakerLindenGridType wavebreaker_get_linden_grid_type(const LLSD& grid, const std::string& grid_key)
{
    std::string key = grid_key;
    LLStringUtil::toLower(key);
    if (key == "util.agni.lindenlab.com")
    {
        return WAVEBREAKER_LINDEN_GRID_AGNI;
    }
    if (key == "util.aditi.lindenlab.com")
    {
        return WAVEBREAKER_LINDEN_GRID_ADITI;
    }

    if (grid.has("gridnick"))
    {
        std::string gridnick = grid["gridnick"].asString();
        LLStringUtil::toLower(gridnick);
        if (gridnick == "agni")
        {
            return WAVEBREAKER_LINDEN_GRID_AGNI;
        }
    ...
    if (grid.has("loginuri") && grid["loginuri"].isArray())
    {
        for (LLSD::array_const_iterator iter = grid["loginuri"].beginArray();
             iter != grid["loginuri"].endArray();
             ++iter)
        {
            std::string login_uri = iter->asString();
            LLStringUtil::toLower(login_uri);
            if (login_uri.find("login.agni.lindenlab.com") != std::string::npos)
            {
                return WAVEBREAKER_LINDEN_GRID_AGNI;
            }
            if (login_uri.find("login.aditi.lindenlab.com") != std::string::npos)
            {
                return WAVEBREAKER_LINDEN_GRID_ADITI;
            }
        }
    ...
    return WAVEBREAKER_LINDEN_GRID_NONE;
}

static bool wavebreaker_is_aditi_grid_name(const std::string& grid_name)
{
    std::string normalized_grid_name = grid_name;
    LLStringUtil::toLower(normalized_grid_name);
    return normalized_grid_name == "aditi" ||
           normalized_grid_name == "util.aditi.lindenlab.com";
}

static void wavebreaker_sanitize_restored_grid_file(const std::string& filename)
{
    if (!LLFile::isfile(filename.c_str()))
    {
        return;
    }
    ...
    LLSD grids;
    {
        llifstream grid_file(filename.c_str(), std::ios::binary);
        if (!grid_file.is_open())
        {
            LL_WARNS("SettingsBackup") << "Unable to open restored grid file for Wavebreaker conversion: " << filename << LL_ENDL;
            return;
        }

        if (LLSDParser::PARSE_FAILURE == LLSDSerialize::fromXMLDocument(grids, grid_file))
        {
            LL_WARNS("SettingsBackup") << "Unable to parse restored grid file for Wavebreaker conversion: " << filename << LL_ENDL;
            return;
        }
    }

    bool changed = false;
    for (LLSD::map_iterator iter = grids.beginMap(); iter != grids.endMap(); ++iter)
    {
        LLSD& grid = iter->second;
        WavebreakerLindenGridType grid_type = grid.isMap() ? wavebreaker_get_linden_grid_type(grid, iter->first) : WAVEBREAKER_LINDEN_GRID_NONE;
        if (grid_type != WAVEBREAKER_LINDEN_GRID_NONE)
        {
            const std::string splash_page = (grid_type == WAVEBREAKER_LINDEN_GRID_ADITI) ? WAVEBREAKER_ADITI_SPLASH_PAGE : WAVEBREAKER_AGNI_SPLASH_PAGE;
            if (!grid.has("loginpage") || !grid["loginpage"].isString() || grid["loginpage"].asString() != splash_page)
            {
                LL_INFOS("SettingsBackup") << "Converting restored "
                    << ((grid_type == WAVEBREAKER_LINDEN_GRID_ADITI) ? "Second Life Beta" : "Second Life")
                    << " grid login page to Wavebreaker local splash for " << iter->first << LL_ENDL;
                grid["loginpage"] = splash_page;
                changed = true;
            }
        }
    }
    ...
    if (changed)
    {
        llofstream grid_file(filename.c_str());
        if (!grid_file.is_open())
        {
            LL_WARNS("SettingsBackup") << "Unable to save converted Wavebreaker grid file: " << filename << LL_ENDL;
            return;
        }
        LLSDSerialize::toPrettyXML(grids, grid_file);
    }
}
...
void FSPanelPreferenceBackup:: doRestoreSettings(const LLSD& notification, const LLSD& response)
{
    ...
    if (gSavedSettings.getBOOL("RestoreGlobalSettings"))
    {
        ...
        bool restored_beta_grid = wavebreaker_is_aditi_grid_name(gSavedSettings.getString("CurrentGrid"));
        gSavedSettings.setBOOL("WavebreakerRestoredSecondLifeBetaGrid", restored_beta_grid);
        LL_INFOS("SettingsBackup") << "Restored settings selected "
            << (restored_beta_grid ? "Second Life Beta grid" : "non-beta or main grid")
            << LL_ENDL;
    ...
    for (size_t index = 0; index < globalFileList.size(); ++index)
    {
        ...
        if (checkbox->getCheckBox()->getValue().asBoolean())
        {
            ...
            std::string file = item->getColumn(2)->getValue().asString();
            LL_INFOS("SettingsBackup") << "copying global file " << file << LL_ENDL;
            std::string target_file = gDirUtilp->getExpandedFilename(LL_PATH_USER_SETTINGS, file);
            copy_prefs_file(
                gDirUtilp->getExpandedFilename(LL_PATH_NONE, dir_name, file),
                target_file);
            if (file == "grids.user.xml")
            {
                wavebreaker_sanitize_restored_grid_file(target_file);
            }
        }
    }
