...
#include "llinventoryfunctions.h"   // for ROOT_WAVEBREAKER_FOLDER
...
{
    // first try to find the #Wavebreaker folder
    LLUUID categoryID = gInventory.findCategoryByName(ROOT_WAVEBREAKER_FOLDER);

    // if no #Wavebreaker folder was found, create one
    if (categoryID.isNull())
    {
        gInventory.createNewCategory(gInventory.getRootFolderID(), LLFolderType::FT_NONE, ROOT_WAVEBREAKER_FOLDER, [this](const LLUUID& new_cat_id)
            {
...
{
    // if still no #Wavebreaker folder was found, try to find the default "Scripts" folder
    if (categoryID.isNull())
    {
