...
void FSAssetBlacklist::loadBlacklist()
{
    ...
    {
        // Try to import old blacklist data from Wavebreaker
        ...
        if (gDirUtilp->fileExists(old_file))
        {
            ...
            if (oldfile.is_open())
            {
                ...
                for (LLSD::map_const_iterator itr = datallsd.beginMap(); itr != datallsd.endMap(); ++itr)
                {
                    ...
                    LLSD newdata;
                    newdata["asset_name"] = "[WAVEBREAKER] " + data["entry_name"].asString();
            ...
            oldfile.close();
            saveBlacklist();
            LL_INFOS("AssetBlacklist") << "Using old Wavebreaker file: " << old_file << LL_ENDL;
        }
        else
        {
