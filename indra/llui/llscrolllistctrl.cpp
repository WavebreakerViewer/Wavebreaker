...
bool LLScrollListCtrl::handleRightMouseDown(S32 x, S32 y, MASK mask)
{
    ...
    if (item)
    {
        ...
        if (! uuid.isNull() && mContextMenuType != MENU_NONE)
        {
            ...
            registrar.add("FS.ZoomIn", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/zoom", true));
            registrar.add("FS.TeleportToTarget", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/teleportto", true));
            registrar.add("FS.OfferTeleport", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/offerteleport", true));
            registrar.add("FS.RequestTeleport", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/requestteleport", true));
            registrar.add("FS.TrackAvatar", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/track", true));
            registrar.add("FS.AddToContactSet", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/addtocontactset", true)); // [FS:CR]
            registrar.add("FS.BlockAvatar", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/blockavatar", true));
            registrar.add("FS.ViewLog", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + id + "/viewlog", true));
            // </FS:Ansariel> Additional convenience options
            ...
            registrar.add("FS.JoinGroup", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + uuid_str + "/groupjoin", true));
            registrar.add("FS.LeaveGroup", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + uuid_str + "/groupleave", true));
            registrar.add("FS.ActivateGroup", std::bind(&LLUrlAction::executeSLURL, "secondlife:///app/wavebreaker/" + uuid_str + "/groupactivate", true));
