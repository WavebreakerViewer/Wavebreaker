...
LLUrlEntrySecondlifeURL::LLUrlEntrySecondlifeURL()
{
    mPattern = boost::regex("((http://([-\\w\\.]*\\.)?(secondlife|lindenlab|tilia-inc)\\.com)"
                            ...
                            "(http://([-\\w\\.]*\\.)?wavebreakerviewer\\.org)"
                            "|"
                            "(https://([-\\w\\.]*\\.)?wavebreakerviewer\\.org(:\\d{1,5})?)"
                            "|"
...
LLUrlEntrySimpleSecondlifeURL::LLUrlEntrySimpleSecondlifeURL()
{
    mPattern = boost::regex("https?://([-\\w\\.]*\\.)?(secondlife|lindenlab|tilia-inc)\\.com(?!\\S)"
                            ...
                            "|"
                            "https?://([-\\w\\.]*\\.)?wavebreakerviewer\\.org(?!\\S)"
                            "|"
...
///
/// FSHelpDebugUrlEntrySL Describes a Wavebreaker Help SLURL, e.g.
...
std::string LLUrlEntryJira::getUrl(const std::string &string) const
{
    ...
    {
        return llformat("https://jira.wavebreakerviewer.org/browse/%s", string.c_str());
    }
    else
    {
