...
bool LLTransUtil::parseStrings(const std::string& xml_filename, const std::set<std::string>& default_args)
{
    ...
    if (!success)
    {
        const std::string error_string =
            "Wavebreaker couldn't access some of the files it needs and will be closed."
            "\n\nPlease reinstall viewer from https://www.wavebreakerviewer.org/download and "
            "contact https://www.wavebreakerviewer.org/support if issue persists after reinstall.";
        LLError::LLUserWarningMsg::show(error_string, LLError::LLUserWarningMsg::ERROR_MISSING_FILES);
        gDirUtilp->dumpCurrentDirectories(LLError::LEVEL_WARN);
        LL_ERRS() << "Couldn't load string table " << xml_filename << ". Please reinstall viewer from https://www.wavebreakerviewer.org/download and contact https://www.wavebreakerviewer.org/support if issue persists after reinstall." << LL_ENDL;
        return false;
    }
