...
int main(int argc, char **argv)
{
    ...
    if( strAsk == "ask" )
    {
        auto choice = fl_choice( "Wavebreaker has crashed, submit the minidump?", "No", "Yes", nullptr );
        if( choice == 0 )
        {
...
/*
    if( curl)
    {
        ...
        curl_mime_data(field, "Wavebreaker-Releasex64", CURL_ZERO_TERMINATED);
...
if (auto curl_handle = curl_easy_init()) {
    ...
    curl_formadd(&formpost, &lastptr,
                 ...
                 CURLFORM_COPYCONTENTS, "Wavebreaker-Releasex64",
                 CURLFORM_END);
