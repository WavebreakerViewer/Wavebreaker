...
echo -e "configure_wavebreaker.sh" > "$LOG"
...
if [ $WANTS_CONFIG -eq $TRUE ] ; then
    ...
    if [ $WANTS_CRASHREPORTING -eq $TRUE ] ; then
        ...
        if [ $TARGET_PLATFORM == "linux" ] ; then
            VIEWER_SYMBOL_FILE="${BUILD_DIR}/newview/wavebreaker-symbols-${TARGET_PLATFORM}-${AUTOBUILD_ADDRSIZE}.tar.bz2"
        else
            VIEWER_SYMBOL_FILE="${BUILD_DIR}/newview/$BTYPE/wavebreaker-symbols-${TARGET_PLATFORM}-${AUTOBUILD_ADDRSIZE}.tar.bz2"
        fi
        CRASH_REPORTING="-DRELEASE_CRASH_REPORTING=ON"
        if [ ! -z $CHANNEL_SIMPLE ]
        then
            CRASH_REPORTING="$CRASH_REPORTING -DUSE_BUGSPLAT=On -DBUGSPLAT_DB=wavebreaker_"`echo $CHANNEL_SIMPLE | tr [:upper:] [:lower:] | sed -e 's/x64//' | sed 's/[^A-Za-z0-9]//g'`
        fi
    else
...
if [ $WANTS_BUILD -eq $TRUE ] ; then
    ...
    if [ $TARGET_PLATFORM == "darwin" ] ; then
        ...
        xcodebuild -configuration $BTYPE -project Wavebreaker.xcodeproj $JOBS 2>&1 | tee -a "$LOG"
        ...
        if [ -f "Wavebreaker.slnx" ]; then
          SOLUTION="Wavebreaker.slnx"
        elif [ -f "Wavebreaker.sln" ]; then
          SOLUTION="Wavebreaker.sln"
        else
          echo "Build failed! No Wavebreaker.slnx or Wavebreaker.sln found"
          exit 1
        fi
        msbuild.exe "$SOLUTION" -p:Configuration=${BTYPE} -flp:LogFile="logs\\WavebreakerBuild_win-${AUTOBUILD_ADDRSIZE}.log" \
            -flp1:"errorsonly;LogFile=logs\\WavebreakerBuild_win-${AUTOBUILD_ADDRSIZE}.err" -p:Platform=${AUTOBUILD_WIN_VSPLATFORM} -t:Build -p:useenv=true \
