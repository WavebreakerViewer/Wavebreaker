...
This procedure is based on discussions with the Wavebreaker Linux development team and is the only one recommended for Wavebreaker for Linux. System requirements are:
...
This will create a folder called wavebreaker and add all the source files. If you desire, you can choose a different folder name by adding the name to the end of the command:

```
git clone https://github.com/WavebreakerViewer/wavebreaker.git NewDestinationDirectory
```
 
The rest of this document will assume the default directory, `wavebreaker`
...
```
export AUTOBUILD_VARIABLES_FILE=$HOME/src/wb-build-variables/variables
...
```
autobuild configure -A 64 -c ReleaseWB_open -- -DLL_TESTS:BOOL=FALSE
autobuild configure -A 64 -c ReleaseWB_open -- --clean
autobuild configure -A 64 -c ReleaseWB_open -- --fmodstudio
autobuild configure -A 64 -c ReleaseWB_open -- --chan="MyBuild"
```

In the last example, the channel and resulting viewer name would be "Wavebreaker-MyBuild". 

The first time you configure, several additional files will be downloaded from Wavebreaker and Second Life sources. These are mostly binary packages maintained outside the viewer development itself. And if you use the `--clean` switch, you will re-download them all.
...
> [!NOTE]
> It is possible to use autobuild to do both the configure step (only needed once) and the build step with one command (`autobuild build -A 64 -c ReleaseWB_open -- --clean [more switches]`). For clarity, they are mentioned separately.
...
> autobuild build -A64 -c ReleaseWB_open -- --package
> ```
...
```
cp ~/src/wavebreaker/build-linux-x86_64/newview/Wavebreaker*.tar.* /path/to/shared/drive
```

or
```
mv ~/src/wavebreaker/build-linux-x86_64/newview/Wavebreaker*.tar.* /path/to/shared/drive
```

When copying the ready-to-run folders and files, use

```
rsync -rptgoDLK --update --progress ~/src/wavebreaker/build-linux-x86_64/newview/packaged/* /path/to/destination
```
...
```
cd /path/to/wavebreaker
etc/refresh_desktop_app_entry.sh
```

Then open your applications menu and look in the Internet or Network branch for the Wavebreaker launcher.
...
- **Wavebreaker Self-Compilers group:** [Wavebreaker Self Compilers](https://tinyurl.com/wavebreaker-self-compilers) is free to join, fellow self-compilers may be able to offer assistance.
- **Jira:** [JIRA](https://jira.wavebreakerviewer.org) may contain resolved issues related to the error you're seeing. Search using the error you encountered. Or create a new issue to report an error in this document, or if a code change causes a build process to fail.
...
- **No sounds:** The viewer will try to use whatever sound service you have running, but might need a little coaxing. Read through the wavebreaker script inside the program directlry, you will find various commented options. Uncommenting one or more may help restore sound, as can compiling with FModStudio. Refer also to the README.Linux.txt and README-linux-voice.txt files in the program directory. 
- **Voice won't connect:** Refer to **[this link](https://wiki.wavebreakerviewer.org/fs_voice#linux)** or the relevant link on **[this page](https://wiki.wavebreakerviewer.org/linux)** to make needed adjustments to your computer and/or the SLVoice files.
