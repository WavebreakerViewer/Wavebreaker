...
> [!WARNING]
> Please note that we do not give support for compiling the viewer on your own. However, there is a self-compilers group in Second Life that can be joined to ask questions related to compiling the viewer: [Wavebreaker Self Compilers](https://tinyurl.com/wavebreaker-self-compilers)
...
### Xcode
XCode is a free download from Apple but you will need an Apple ID to login and access the download section. The current version used to compile Wavebreaker is 16.4.
...
If you want to use licensed FMOD Studio API or KDU (short for Kakadu) build libraries (they are optional) you have to provide these yourself. If you're building Wavebreaker as part of the project team, ask for the libraries for fmodstudio and kdu. Put them into `/opt/wavebreaker`. Some libraries like Kakadu or Havok require that you purchase a license so do not set the flags to include those packages if you do not have the according licenses.
...
We've created a non-KDU build target to make this easier. Everywhere you see "ReleaseWB" below, use "ReleaseWB_open" instead. This will perform the same build, using openjpeg instead of KDU and omitting FMOD Studio.

You will probably want to have FMOD enabled and no Kakadu, in that case, you can use the ReleaseWB_open target with the --fmodstudio switch.

##  Set up your source code tree

Plan your directory structure ahead of time. If you are going to be producing changes or patches you will be cloning a copy of an unaltered source code tree for every change or patch you make, so you might want to have all this work stored in its own directory. If you are a casual compiler and won't be producing any changes, you can use one directory. For this document, we will assume $HOME/wavebreaker.

```
mkdir ~/wavebreaker
cd ~/wavebreaker
git clone https://github.com/WavebreakerViewer/wavebreaker.git
```

This can take a while. It's a rather large download.

You will also need to download the build variables used for building the viewer. Like the viewer source, these are downloaded from the Wavebreaker git repository. Assuming you are still in your ~/wavebreaker directory (or wherever else you chose), issue the following commands:

```
git clone https://github.com/WavebreakerViewer/wb-build-variables.git
```

You will then need to add this line to your ~/.zshrc file (assuming you are on Catalina or later):
```
echo 'AUTOBUILD_VARIABLES_FILE=~/wavebreaker/wb-build-variables/variables' | sudo tee ~/.zshrc
```

Again, if you do not wish to restart your terminal:
```
export AUTOBUILD_VARIABLES_FILE=~/wavebreaker/wb-build-variables/variables
```

## Wavebreaker build targets
Available premade wavebreaker-specific build targets:
...
To build wavebreaker:

```
autobuild build -c ReleaseWB                        
```

Other examples:

```
autobuild configure -c ReleaseWB                              # basic configuration step, don't build, just configure
autobuild configure -c ReleaseWB -- --clean                   # clean the output area first, then configure
autobuild configure -c ReleaseWB -- --chan Private-Yourname   # configure with a custom channel

autobuild build -c ReleaseWB --no-configure                   # default quick rebuild
autobuild build -c ReleaseWB --no-configure -- --clean        # Clean rebuild

autobuild configure -c ReleaseWB_open --                      # configure with no third-party libraries
autobuild configure -c ReleaseWB_open -- --fmodstudio         # configure with FMOD Studio but no KDU
```
...
Most third party libraries needed to build the viewer will be automatically downloaded for you and installed into the build directory within your source tree during compilation. Some need to be manually prepared and are not normally required when using an open source configuration (ReleaseWB_open).
...
```
git clone https://github.com/WavebreakerViewer/3p-fmodstudio.git
cd 3p-fmodstudio
```
...
- Next, make a copy of Wavebreakers autobuild.xml named for example my_autobuild.xml to use your FMOD Studio.
...
```
autobuild configure -A 64 -c ReleaseWB_open -- -DLL_TESTS:BOOL=FALSE -DFMODSTUDIO:BOOL=TRUE
```
```
autobuild configure -A 64 -c ReleaseWB_open
```
...
> [!NOTE]
> It is possible to use autobuild to do both the configure step (only needed once) and the build step with one command (`autobuild build -A 64 -c ReleaseWB[_open] [-- config options]`). Some find it is clearer if these steps are done separately, but can save a bit of time if done together.
...
- First you need to change to the directory where you built the viewer before. Assuming you followed the steps above and used the same names, run the following in the terminal window: `cd ~/wavebreaker/wavebreaker`
...
- After any new code is downloaded, it needs to be reconfigured. Run the following in the terminal window: `autobuild configure -A 64 -c ReleaseWB_open`
- Finally, re-compile the viewer with the new changes. Again, in the terminal window, run: `autobuild build -A 64 -c ReleaseWB_open --no-configure`
