# Build instructions for Wavebreaker on Windows

This page is derived from the upstream Firestorm Windows build instructions and adjusted for a Wavebreaker tree produced with the local Wavebreaker patch scripts. It describes the steps to build the Wavebreaker viewer for Windows. For building instructions up to (and including) release 6.5.3, see the archived version for [building with Python 2.7](https://wiki.firestormviewer.org/archive:fs_compiling_firestorm_windows_py_27).
...
- Download and install the most recent version of [Python 3](https://www.python.org/downloads/windows)
  - Run the installer as Administrator (right click, â€œRun as administratorâ€)
...
## Setup Wavebreaker build variables

Autobuild expects a variable-definition file. For Wavebreaker, keep the helper repository under the Wavebreaker source tree as `wb-build-variables`.

If you are starting from a fresh setup:

```cmd
c:
cd \wavebreaker\wavebreaker
git clone https://github.com/FirestormViewer/fs-build-variables.git wb-build-variables
```

Set `AUTOBUILD_VARIABLES_FILE` to the actual variables file:

```cmd
set AUTOBUILD_VARIABLES_FILE=C:\wavebreaker\wavebreaker\wb-build-variables\variables\variables
```

If you are running `autobuild configure` from Cygwin/bash, use an exported POSIX path instead:

```bash
export AUTOBUILD_VARIABLES_FILE=/cygdrive/g/firestorm/codex_builds/1/wavebreaker/wb-build-variables/variables/variables
test -f "$AUTOBUILD_VARIABLES_FILE"
```

The `export` matters. A plain shell assignment can be visible to `echo` but invisible to child processes such as `autobuild`.

If CMake reports `Environment variable LL_BUILD must be set` after selecting `ReleaseWB_open`, rerun `rename-wavebreaker-autobuild-configs.ps1`. The Autobuild configuration name and the `LL_BUILD_*_RELEASEWB*` variable names in `wb-build-variables` must match.

For a permanent user environment variable, use:

```cmd
setx AUTOBUILD_VARIABLES_FILE C:\wavebreaker\wavebreaker\wb-build-variables\variables\variables
```

If you already ran `rename-wavebreaker-build-variables.ps1`, verify the folder exists here:

```cmd
dir C:\wavebreaker\wavebreaker\wb-build-variables\variables
```
...
## Set up your Wavebreaker source code tree

Plan your directory structure ahead of time. This document assumes you are working under `C:\wavebreaker` and that the Wavebreaker patch scripts are available in that folder. Copy `prepare-wavebreaker-source.ps1`, `patch-wavebreaker-source-tree.ps1`, `rename-wavebreaker-build-variables.ps1`, and `rename-wavebreaker-autobuild-configs.ps1` into `C:\wavebreaker` before running the patch commands.

Start from current Firestorm source, then create the rebranded Wavebreaker tree:

```cmd
c:
mkdir \wavebreaker
cd \wavebreaker
git clone https://github.com/FirestormViewer/phoenix-firestorm.git phoenix-firestorm
```

Run the Wavebreaker patch scripts from PowerShell:

```powershell
cd C:\wavebreaker
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\prepare-wavebreaker-source.ps1 -Force
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\patch-wavebreaker-source-tree.ps1 -SourceRoot .\wavebreaker -Force
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\rename-wavebreaker-build-variables.ps1 -SourceRoot .\wavebreaker -Force
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\rename-wavebreaker-autobuild-configs.ps1 -SourceRoot .\wavebreaker -Force
```

Expected source folder after patching:

```cmd
C:\wavebreaker\wavebreaker
```

## Prepare third party libraries

Most third party libraries needed to build the viewer will be automatically downloaded for you and installed into the build directory within your source tree during compilation. Some need to be manually prepared and are not normally required when using an open source configuration (ReleaseWB_open). Some libraries like Kakadu or Havok requires you to purchase a license and you will need to figure out yourself how to build and use them.
...
```
wrote  C:\wavebreaker\3p-fmodstudio\fmodstudio-{version#}-windows64-{build_id}.tar.bz2''
```
...
`autobuild installables edit fmodstudio platform=windows64 hash=a0d1821154e7ce5c418e3cdc2f26f3fc url=file:///C:/wavebreaker/3p-fmodstudio/fmodstudio-2.01.02-windows-192171947.tar.bz2`
...
This will configure Wavebreaker to be built with all defaults and without third party libraries. The rebrand patch sets the viewer binary name to `wavebreaker-bin`; if you configure CMake manually, pass `-DVIEWER_BINARY_NAME=wavebreaker-bin`.

Available premade Wavebreaker/Firestorm-derived build targets:

```
ReleaseWB             (with KDU, with FMOD,   no OpenSim)
ReleaseWB_AVX         (with KDU, with FMOD,   no OpenSim, optimized for AVX-enabled CPUs)
ReleaseWB_AVX2        (with KDU, with FMOD,   no OpenSim, optimized for AVX2-enabled CPUs)
ReleaseWB_open        (  no KDU,   no FMOD,   no OpenSim)
ReleaseOS             (  no KDU,   no FMOD, with OpenSim)
RelWithDebInfoWB      (with KDU, with FMOD,   no OpenSim, with debug info)
RelWithDebInfoWB_open (  no KDU,   no FMOD,   no OpenSim, with debug info)
...
> `autobuild configure -A 64 -v -c ReleaseWB_open`
...
- **--chan \<channel name\>** will set a unique channel (and the name) for the viewer, appending whatever is defined to "Wavebreaker-". By default, the channel is "private" followed by your computer's name.
...
`autobuild configure -A 64 -c ReleaseWB_open -- --fmodstudio --package --chan MyViewer -DLL_TESTS:BOOL=FALSE`

- To build a 64bit viewer without FMOD Studio and without installer package, run this command:
`autobuild configure -A 64 -c ReleaseWB_open -- --chan MyViewer -DLL_TESTS:BOOL=FALSE`
...
`autobuild build -A 64 -c ReleaseWB_open --no-configure`

Compiling will take quite a bit of time.

If MSVC reports `C3859`, `C1076`, `Failed to create virtual memory for PCH`, or `The paging file is too small`, the failure is resource-related. Retry with single-threaded MSBuild or increase the Windows page file:

```cmd
cmake --build C:\wavebreaker\wavebreaker\build-vc170-64 --target wavebreaker-bin --config Release -- /m:1 /p:CL_MPCount=1
```

### Building from within Visual Studio

Inside the Wavebreaker source folder, you will find a folder named build-vc170-\<architecture\>, with \<architecture\> either being 32 or 64, depending on what you chose during the configuration step. Inside the folder is the generated Visual Studio solution file.

- Open the generated `.sln` file in `build-vc170-64`. Depending on the generator/cache, it may be named `Wavebreaker.sln` or retain the original generated solution name; build the `wavebreaker-bin` target if present.
